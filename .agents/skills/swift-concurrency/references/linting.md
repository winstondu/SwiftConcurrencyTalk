...
## Concurrency Lint Rules Overview

Linters often provide rules targeting async/await and concurrency patterns. Understanding when to fix vs. suppress is critical.
...
```swift
// lint:disable:next async_without_await
func fetch() async { perform() }

// For a block:
// lint:disable async_without_await
func makeMock() async { perform() }
// lint:enable async_without_await
```
